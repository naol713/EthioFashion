  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-gray-600 mt-1">Monitor available stock and record adjustments.</p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-left text-gray-600 font-semibold">
            <tr>
              ...
              <th className="p-4">Adjust &nbsp;/&nbsp; Note &nbsp;/&nbsp; Save</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {inventory.length === 0 && (
              <tr>
                <td colSpan={7} className="p-6 text-center text-gray-400">No inventory records found.</td>
              </tr>
            )}
            {inventory.map((item) => {
              ...
              return (
                <tr key={item.id} className={isLow ? 'bg-red-50' : ''}>
                  ...
                  <td className="p-4 font-mono text-xs text-gray-600">{item.variant.sku}</td>
                  <td className="p-4 text-center">{item.quantity}</td>
                  <td className="p-4 text-center text-amber-600">{item.reserved_quantity}</td>
                  <td className={`p-4 text-center font-bold ${isLow ? 'text-red-600' : 'text-green-700'}`}>
                    {available}
                    {isLow && (
                  ...
                  <td className="p-4 text-center text-gray-500">{item.low_stock_threshold}</td>
                  <td className="p-4">
