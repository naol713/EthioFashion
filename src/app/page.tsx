...
export default async function Home() {
  const [featuredRes, categoriesRes, productCount, customerCount] =
    await Promise.all([
  ...
  const categories = Array.isArray(categoriesRes.data)
    ? categoriesRes.data.slice(0, 4)
    : [];
  const wishlistedIds = new Set(
    wishlistRes.success
      ? wishlistRes.items.map((item: any) => item.product_id)
      : [],
  );
            ...
            <Container>
              <div className="flex items-end justify-between mb-12">
                ...
                <Button
                  variant="outline"
                  asChild
                  className="hidden sm:flex border-[#0a0a0a] text-[#0a0a0a] hover:bg-[#0a0a0a] hover:text-white"
                >
            ...
            <div className="grid items-center gap-14 lg:grid-cols-[0.95fr_1.05fr]">
              <div className="max-w-xl">
                ...
                <p className="mt-6 max-w-xl text-base leading-8 text-gray-600 sm:text-lg">
                  We partner with Ethiopian artisans, weavers, and shoemakers to
                  bring you authentic, high-quality fashion that celebrates our
                  culture while meeting modern standards of quality and design.
                </p>
                  ...
                  <div className="grid grid-cols-3 gap-4 sm:gap-6">
                    {[
                      ...
                      },
                      {
                        value:
                          customerCount >= 1000
                            ? `${(customerCount / 1000).toFixed(1).replace(/\.0$/, "")}K+`
                            : `${customerCount}+`,
          ...
          <Container>
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
              ...
              <div className="flex flex-wrap items-center gap-4">
                {["Telebirr", "Chapa", "CBE Birr", "Bank of Abyssinia"].map(
                  (method) => (
                    <div
                      key={method}
