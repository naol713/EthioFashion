...
export const metadata: Metadata = {
  ...
  description:
    "Browse premium Ethiopian fashion — clothing, shoes, and accessories.",
};
...
export default async function ProductsPage({
  ...
  const activeFiltersCount = [categorySlug, brandSlug, gender, query].filter(
    Boolean,
  ).length;
            ...
            {activeFiltersCount > 0 && (
              <div className="flex flex-wrap gap-2">
                {categorySlug && (
                  <Badge
                    variant="secondary"
                    className="bg-white/10 text-white/80 border-0 gap-1.5 pr-1.5"
                  >
                    {categorySlug}
                    <Link
                      href={buildUrl({ category: undefined })}
                      className="hover:text-white ml-1 text-white/50"
                    >
                      ×
                    </Link>
                  </Badge>
        ...
        <div className="flex gap-8">
          ...
          <aside className="hidden lg:block w-60 flex-shrink-0">
            ...
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden divide-y divide-gray-100">
              ...
              {/* Gender accordion */}
              <details open={!!gender} className="group">
                <summary className="flex items-center justify-between px-4 py-3 cursor-pointer select-none list-none">
                  <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400">
                    Gender
                  </span>
              ...
              {/* Brands accordion */}
              {brands.length > 0 && (
                <details open={!!brandSlug} className="group">
                  <summary className="flex items-center justify-between px-4 py-3 cursor-pointer select-none list-none">
                    <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400">
                      Brands
                    </span>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-500">
                  {products.length} of {pagination?.total ?? products.length}{" "}
                  products
                </span>
              </div>
              <div className="flex items-center gap-2">
            ...
            <details className="lg:hidden mb-6 bg-white border border-gray-200 rounded-xl overflow-hidden">
              <summary className="flex items-center justify-between px-4 py-3 cursor-pointer select-none text-sm font-semibold text-[#0a0a0a]">
                <span className="flex items-center gap-2">
                  ...
                  Filters
                  {activeFiltersCount > 0 && (
                    <Badge className="bg-[#D4AF37] text-black text-[10px] border-0 px-1.5">
                      {activeFiltersCount}
                    </Badge>
                  )}
                </span>
