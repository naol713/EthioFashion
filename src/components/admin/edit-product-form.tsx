...
export function EditProductForm({ product, categories, brands }: EditProductFormProps) {
  ...
  const [isSlugTouched, setIsSlugTouched] = useState(true); // Default to true since it already exists
  ...
  const compareAtPrice = firstVariant?.compare_at_price ? Number(firstVariant.compare_at_price) : '';
  const stockQuantity = firstVariant?.inventory?.quantity ?? 0;
  const sizeType = (firstVariant?.size?.type || 'CLOTHING') as 'CLOTHING' | 'SHOE';
  const initialSizes = activeVariants.map((v) => v.size?.name).filter((name): name is string => !!name);
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-xl border p-6 space-y-4">
      ...
      <ProductSizeFields initialSizeType={sizeType} initialSizes={initialSizes} />

      <fieldset className="grid grid-cols-1 sm:grid-cols-2 gap-3 border-0 p-0 m-0" disabled={isSubmitting}>
         <div>
           <label className="block text-sm font-semibold text-gray-700 mb-1">Category *</label>
           <select name="categoryId" defaultValue={product.category_id} className="w-full h-10 border rounded-md px-3" required>
         ...
         <div>
           ...
           <select name="brandId" defaultValue={product.brand_id || ''} className="w-full h-10 border rounded-md px-3">
         ...
         <div>
           ...
           <select name="gender" defaultValue={product.gender || ''} className="w-full h-10 border rounded-md px-3">
         ...
         <div>
           ...
           <select name="status" defaultValue={product.status} className="w-full h-10 border rounded-md px-3">
